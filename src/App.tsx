...
export default function App() {
  ...
  const scopedRequests = requests.filter((r) => {
    ...
    // Timika should see everything they submitted, Admin sees all
    return true; 
  });
  ...
  const filterTabRequests = (status: RequestStatus | RequestStatus[]) => {
    return scopedRequests.filter((r) => {
      // Allow passing an array of statuses to combine logic (e.g. for Timika's Active view)
      if (Array.isArray(status)) {
        if (!status.includes(r.status)) return false;
      } else {
  ...
  // Timika views their entire active pipeline (Awaiting + In Progress) combined
  const timikaActiveRequestsFiltered = filterTabRequests([RequestStatus.WAITING, RequestStatus.IN_PROGRESS]);
  
  // Specific splits for Workshops
  const awaitingRequestsFiltered = filterTabRequests(RequestStatus.WAITING);
  const inProgressRequestsFiltered = filterTabRequests(RequestStatus.IN_PROGRESS);
  const completedRequestsFiltered = filterTabRequests(RequestStatus.DONE);
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 flex flex-col text-slate-900 font-sans pb-20 sm:pb-0 w-full overflow-x-hidden print:bg-white print:overflow-visible print:min-h-0 print:pb-0 print:block">
      ...
      <div className="print:hidden flex-1 flex flex-col w-full">
        ...
        <div className="sticky top-0 z-40 bg-slate-900 border-b border-slate-800 px-4 sm:px-6 lg:px-8 py-2.5 flex items-center justify-between shrink-0 shadow-md">
          ...
          <div className="flex items-center space-x-3 sm:hidden">
            <button
              ...
              className="p-2 text-slate-300 hover:text-white bg-slate-800 hover:bg-slate-700 rounded-xl transition-colors cursor-pointer flex items-center space-x-2"
            >
              <Menu className="h-5 w-5" />
              <span className="text-xs font-bold uppercase tracking-wider">{currentTab}</span>
            </button>
          </div>
          ...
          <div className="hidden sm:flex flex-wrap items-center gap-2 w-full sm:w-auto">
            ...
            {(currentRole === LocationTeam.SURABAYA || currentRole === LocationTeam.JAKARTA) && (
              <>
                ...
                >
                  ...
                  <span>Awaiting Repair ({waitingTickets})</span>
                </button>

                <button
                  onClick={() => setCurrentTab("in-progress")}
                  className={`flex items-center justify-center space-x-1.5 px-3 py-2.5 sm:px-3.5 rounded-xl text-[10px] sm:text-xs font-extrabold uppercase tracking-wider transition-all cursor-pointer ${
                    currentTab === "in-progress" ? "bg-blue-600 text-white shadow-md" : "text-slate-400 hover:text-white hover:bg-slate-800"
                  }`}
                >
            ...
            {/* Same layout behavior applied to Timika and Admin view, rendering the individual Queue tabs */}
            {(currentRole === LocationTeam.TIMIKA || currentRole === "Admin") && (
              <>
                ...
                <button
                  onClick={() => setCurrentTab("in-progress")}
                  className={`flex items-center justify-center space-x-1.5 px-3 py-2.5 sm:px-4 rounded-xl text-[10px] sm:text-xs font-extrabold uppercase tracking-wider transition-all cursor-pointer ${
                ...
                >
                  ...
                  <span>{language === "ENG" ? "Active Pipeline" : "Pipa Aktif"} ({waitingTickets + activeRepairs})</span>
                </button>
        ...
        {/* Mobile Left Sidebar Navigation Menu (Overlay Drawer - Never Takes Up Full Screen) */}
        {isMobileMenuOpen && (
          <div className="fixed inset-0 z-50 flex sm:hidden">
            ...
            />
            <div className="relative flex flex-col w-64 max-w-[75vw] bg-slate-900 border-r border-slate-800 p-5 space-y-4 h-full shadow-2xl animate-fade-in z-10">
              <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                <span className="text-xs font-bold font-mono uppercase tracking-wider text-slate-400">Navigation Menu</span>
              ...
              <div className="flex flex-col space-y-2">
                <button
                  ...
                  className={`flex items-center space-x-3 px-3.5 py-3 rounded-xl text-xs font-bold uppercase transition-all text-left ${
                    currentTab === "dashboard" ? "bg-blue-600 text-white shadow-md" : "text-slate-300 hover:bg-slate-800"
                  }`}
                >
                ...
                {(currentRole === LocationTeam.TIMIKA || currentRole === "Admin") && (
                  <>
                    ...
                    >
                      <BarChart3 className="h-4 w-4 shrink-0 text-blue-400" />
                      <span>Active Pipeline ({waitingTickets + activeRepairs})</span>
                    </button>
                ...
                <button
                  ...
                  className={`flex items-center space-x-3 px-3.5 py-3 rounded-xl text-xs font-bold uppercase transition-all text-left ${
                    currentTab === "history" ? "bg-blue-600 text-white shadow-md" : "text-slate-300 hover:bg-slate-800"
                  }`}
                >
            ...
            {currentTab === "dashboard" ? (
              ...
              <div className="space-y-6">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-4">
                    ...
                    <div className="flex items-center space-x-2">
                      <span className="bg-amber-100 text-amber-800 px-3 py-1 rounded-full text-xs font-mono font-bold">
                        {awaitingRequestsFiltered.length} Records Found
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-3">
                    <div className="relative flex-1 w-full">
                      ...
                      <input
                        ...
                        placeholder="Search awaiting container unit (e.g., FUKU-610012-2) or ID..."
                        className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-mono focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500/20"
                      />
                      {searchQuery && (
                        <button
                ...
                <div className="space-y-3">
                  {awaitingRequestsFiltered.length === 0 ? (
                    <div className="py-12 text-center text-slate-400 font-mono text-xs uppercase bg-white rounded-2xl border border-slate-200">No matching awaiting requests</div>
                  ) : (
                    awaitingRequestsFiltered.map((req) => (
                      <div key={req.id} onClick={() => setSelectedRequest(req)} className="p-4 rounded-xl border border-slate-200 hover:border-amber-400 transition cursor-pointer bg-white shadow-sm flex justify-between items-center">
                        <div>
              ...
              /* TAB VIEW: IN PROGRESS WITH FULL SEARCH & FILTERING (COMBINED FOR TIMIKA) */
              <div className="space-y-6">
                <div className="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm space-y-4">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-1">
                        {currentRole === LocationTeam.TIMIKA || currentRole === "Admin"
                          ? "All containers currently awaiting or undergoing workshop intervention."
                          : "Containers currently undergoing workshop intervention."}
                      </p>
                    </div>
                    <div className="flex items-center space-x-2">
                      <span className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-xs font-mono font-bold">
                        {currentRole === LocationTeam.TIMIKA || currentRole === "Admin" 
                          ? timikaActiveRequestsFiltered.length 
                          : inProgressRequestsFiltered.length} Records Found
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-3">
                    <div className="relative flex-1 w-full">
                      ...
                      <input
                        ...
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        placeholder="Search active container unit (e.g., FUKU-610012-2) or ID..."
                ...
                <div className="space-y-3">
                  {(currentRole === LocationTeam.TIMIKA || currentRole === "Admin" ? timikaActiveRequestsFiltered : inProgressRequestsFiltered).length === 0 ? (
                    <div className="py-12 text-center text-slate-400 font-mono text-xs uppercase bg-white rounded-2xl border border-slate-200">No matching active requests</div>
                  ) : (
                    (currentRole === LocationTeam.TIMIKA || currentRole === "Admin" ? timikaActiveRequestsFiltered : inProgressRequestsFiltered).map((req) => (
                      <div key={req.id} onClick={() => setSelectedRequest(req)} className="p-4 rounded-xl border border-slate-200 hover:border-blue-400 transition cursor-pointer bg-white shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        <div>
                        ...
                        <span className={`px-2.5 py-1 rounded-full text-[10px] font-mono font-bold uppercase w-fit ${
                          req.status === RequestStatus.WAITING ? "bg-amber-100 text-amber-800" : "bg-blue-100 text-blue-800"
                        }`}>
                          {req.status}
                        </span>
                      </div>
              ...
              /* TAB VIEW: COMPLETED QUEUE WITH FULL SEARCH & FILTERING */
              <div className="space-y-6">
                <div className="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm space-y-4">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-4">
                    <div>
                    ...
                    <div className="flex items-center space-x-2">
                      <span className="bg-emerald-100 text-emerald-800 px-3 py-1 rounded-full text-xs font-mono font-bold">
                        {completedRequestsFiltered.length} Records Found
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-3">
                    <div className="relative flex-1 w-full">
                      ...
                      <input
                        ...
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        placeholder="Search completed container unit (e.g., FUKU-610012-2) or ID..."
                        className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-mono focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/20"
                      />
                      {searchQuery && (
                        <button
                          onClick={() => setSearchQuery("")}
                ...
                <div className="space-y-3">
                  {completedRequestsFiltered.length === 0 ? (
                    <div className="py-12 text-center text-slate-400 font-mono text-xs uppercase bg-white rounded-2xl border border-slate-200">No matching completed requests</div>
                  ) : (
                    completedRequestsFiltered.map((req) => (
                      <div key={req.id} onClick={() => setSelectedRequest(req)} className="p-4 rounded-xl border border-slate-200 hover:border-emerald-400 transition cursor-pointer bg-white shadow-sm flex justify-between items-center">
                        <div>
