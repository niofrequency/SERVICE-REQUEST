      ...
      <div className="print:hidden flex-1 flex flex-col w-full">
        ...
        <div className="sticky top-0 z-40 bg-slate-900 border-b border-slate-800 px-4 sm:px-6 lg:px-8 py-2.5 flex items-center justify-between shrink-0 shadow-md">
          ...
          <div className="hidden sm:flex flex-wrap items-center gap-2 w-full sm:w-auto">
            ...
            {/* Show Awaiting, In Progress, and Completed tabs for Timika, Surabaya, Jakarta, and Admin */}
            {(currentRole === LocationTeam.SURABAYA || currentRole === LocationTeam.JAKARTA || currentRole === LocationTeam.TIMIKA || currentRole === "Admin") && (
              <>
                <button
        ...
        <main className="flex-1 max-w-[1920px] w-full mx-auto p-4 sm:p-6 lg:px-8 space-y-6 relative min-h-[500px]">
          ...
          <div className={`space-y-6 ${isLoading ? "blur-sm pointer-events-none opacity-80 select-none" : "animate-fade-in"}`}>
            ...
            {currentTab === "dashboard" ? (
                ...
                <div className="space-y-3">
                  {awaitingRequestsFiltered.length === 0 ? (
                    ...
                    awaitingRequestsFiltered.map((req) => {
                      const displayLocation = req.location === LocationTeam.TIMIKA && req.destinationLocation ? req.destinationLocation : req.location || "UNKNOWN HUB";
                      return (
                        <div key={req.id} onClick={() => setSelectedRequest(req)} className="p-4 rounded-xl border border-slate-200 hover:border-amber-400 transition cursor-pointer bg-white shadow-sm flex justify-between items-center">
                          <div>
                            <div className="flex items-center space-x-2">
                              ...
                              <span className="px-1.5 py-0.5 rounded bg-slate-100 border border-slate-200 text-[9px] font-bold text-slate-600 uppercase tracking-wider">{displayLocation}</span>
                            </div>
              ...
              /* TAB VIEW: IN PROGRESS WITH FULL SEARCH & FILTERING */
              <div className="space-y-6">
                <div className="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm space-y-4">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-4">
                    <div>
                      <h2 className="text-lg font-extrabold text-slate-900 uppercase tracking-tight flex items-center space-x-2.5">
                        ...
                        <span>Active In-Progress Repairs</span>
                      </h2>
                      <p className="text-xs text-slate-500 mt-1">Containers currently undergoing workshop intervention.</p>
                    </div>
                    <div className="flex items-center space-x-2">
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-3">
                    <div className="relative flex-1 w-full">
                      ...
                      <input
                        ...
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        placeholder="Search in-progress container unit (e.g., FUKU-610012-2) or ID..."
                ...
                <div className="space-y-3">
                  {inProgressRequestsFiltered.length === 0 ? (
                    <div className="py-12 text-center text-slate-400 font-mono text-xs uppercase bg-white rounded-2xl border border-slate-200">No matching in-progress requests</div>
                  ) : (
                    inProgressRequestsFiltered.map((req) => {
                      const displayLocation = req.location === LocationTeam.TIMIKA && req.destinationLocation ? req.destinationLocation : req.location || "UNKNOWN HUB";
                      return (
                        <div key={req.id} onClick={() => setSelectedRequest(req)} className="p-4 rounded-xl border border-slate-200 hover:border-blue-400 transition cursor-pointer bg-white shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                          <div>
                            <div className="flex items-center space-x-2 font-mono text-xs font-bold">
                              <span className="text-blue-600">{req.id}</span>
                              <span className="text-slate-400">•</span>
                              <span className="text-slate-900">{req.containerNumber}</span>
                              <span className="px-1.5 py-0.5 rounded bg-slate-100 border border-slate-200 text-[9px] font-bold text-slate-600 uppercase tracking-wider font-sans">{displayLocation}</span>
                            </div>
                            <p className="text-xs text-slate-600 mt-1">{req.description}</p>
                          </div>
                          <span className="px-2.5 py-1 rounded-full text-[10px] font-mono font-bold uppercase bg-blue-100 text-blue-800 w-fit">
                            {req.status}
                          </span>
                        </div>
                      );
